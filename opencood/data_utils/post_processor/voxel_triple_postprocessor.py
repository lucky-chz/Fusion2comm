...
class VoxelTriplePostprocessor(BasePostprocessor):
    def __init__(self, anchor_params, train):
        super(VoxelTriplePostprocessor, self).__init__(anchor_params, train)
    ...
    def generate_label_add_occ(self, **kwargs):
        """
        Generate targets for training.
        ...
        # (max_num, 1)
        occlusion_state = kwargs['occlusion_state']
        occlusion_state = 1 - occlusion_state/2
        # (H, W)
        feature_map_shape = anchors.shape[:2]
        ...
        # (n, 1)
        # occlusion_state_valid = occlusion_state[masks == 1]
        # occlusion_state_valid = torch.from_numpy(occlusion_state_valid)
        # occlusion_state_one_hot = torch.nn.functional.one_hot(occlusion_state_valid.to(torch.int64), occlusion_num) 
        # one_hot_sum_max = occlusion_state_one_hot.argmax(dim=1, keepdim=True)
        ...
        index_x, index_y, index_z = np.unravel_index(
            id_pos, (*feature_map_shape, self.anchor_num))
        pos_equal_one[index_x, index_y, index_z] = 1
        occ_equal_one = pos_equal_one * np.expand_dims(occlusion_state, axis=2)
        # calculate the targets
        targets[index_x, index_y, np.array(index_z) * 7] = \
            (gt_box_center[id_pos_gt, 0] - anchors[id_pos, 0]) / anchors_d[
                id_pos]
    ...
    def fuse3dboxes(self, boxes3d, boxes3d_single_vi, score, score_vi, thresh=0.3, min_thresh=0.25):
        dis = torch.norm(boxes3d[:, :3].unsqueeze(1) - boxes3d_single_vi[:, :3].unsqueeze(0), p=2, dim=-1)
        mask = dis.min(dim=0)[0]>thresh
        boxes3d_single_vi = boxes3d_single_vi[mask]
        score_vi = score_vi[mask]
        mask_range = (boxes3d_single_vi[:, 0]>self.params['anchor_args']['cav_lidar_range'][0]) & \
                     (boxes3d_single_vi[:, 0]<self.params['anchor_args']['cav_lidar_range'][3]) & \
                     (boxes3d_single_vi[:, 1]>self.params['anchor_args']['cav_lidar_range'][1]) & \
                     (boxes3d_single_vi[:, 1]<self.params['anchor_args']['cav_lidar_range'][4]) & \
                     (boxes3d_single_vi[:, 2]>self.params['anchor_args']['cav_lidar_range'][2]) & \
                     (boxes3d_single_vi[:, 2]<self.params['anchor_args']['cav_lidar_range'][5])
        boxes3d_single_vi = boxes3d_single_vi[mask_range]
        score_vi = score_vi[mask_range] * 0.5
        score_vi[score_vi<min_thresh] = 0.25
        boxes3d = torch.concat([boxes3d, boxes3d_single_vi])
        score = torch.concat([score, score_vi])
        return boxes3d, score
    ...
    def post_process(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            boxes3d, scores = self.fuse3dboxes(boxes3d, 
                                               boxes3d_single_vi, 
                                               scores, 
                                               scores_vi,
                                               min_thresh=self.params['target_args']['score_threshold'])
