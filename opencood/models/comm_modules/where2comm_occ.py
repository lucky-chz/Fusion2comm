...
class Communication_occ(nn.Module):
    def __init__(self, args):
        super(Communication_occ, self).__init__()
    ...
    def forward(self, batch_confidence_maps, batch_occlusion_maps, record_len, pairwise_t_matrix):
        ...
        for b in range(B):
            ...
            ori_communication_maps = (batch_confidence_maps[b].sigmoid().max(dim=1)[0].unsqueeze(1))*(batch_occlusion_maps[b].sigmoid().max(dim=1)[0].unsqueeze(1)) # dim1=2 represents the confidence of two anchors
