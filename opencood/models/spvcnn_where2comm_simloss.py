...
class SPVCNNWhere2commSimLoss(nn.Module):
    def __init__(self, args, train=True):
        super(SPVCNNWhere2commSimLoss, self).__init__()
        self.lambda_xm = args['lambda_xm']
    ...
    def forward(self, data_dict, vis=False):
        ...
        for b in range(B):
            ...
            psm_single_ego_b = warp_affine_simple(batch_node_psmsingle[b],
                                            t_matrix[0, :, :, :],
                                            (H, W))
            psm_single_ego_b = torch.max(psm_single_ego_b, dim=0)[0]
        ...
        psm_fuse = torch.cat([psm.unsqueeze(0), psm_single_ego.unsqueeze(0)], dim=0)
        psm_fuse = torch.max(psm_fuse, dim=0)[0]
        cls_preds = psm.permute(0, 2, 3, 1).contiguous()
        cls_preds = cls_preds.view(-1, 1)
        cls_preds_fuse = psm_fuse.permute(0, 2, 3, 1).contiguous()
        cls_preds_fuse = cls_preds_fuse.view(-1, 1)
        cls_sim_loss = F.mse_loss(
            cls_preds,
            cls_preds_fuse,
            reduction='mean'
        ) * self.lambda_xm
