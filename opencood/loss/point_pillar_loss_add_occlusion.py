...
class PointPillarLossAddOcclusion(nn.Module):
    def __init__(self, args):
        super(PointPillarLossAddOcclusion, self).__init__()
        self.reg_loss_func = WeightedSmoothL1Loss()
        self.alpha = 0.25
        self.gamma = 2.0

        self.cls_weight = args['cls_weight']
        self.occ_weight = args['occlusion_weight']
    ...
    def forward(self, output_dict, target_dict, prefix=''):
        ...
        # add occ loss
        if 'occ_equal_one' in target_dict:
            occ = output_dict['occ{}'.format(prefix)]
            box_occ_labels = target_dict['occ_equal_one']  # [B, 50, 176, 2]
            box_occ_labels = box_occ_labels.view(occ.shape[0], -1).contiguous()
            occ_preds = occ.permute(0, 2, 3, 1).contiguous()
            occ_targets = box_occ_labels
            occ_targets = occ_targets.unsqueeze(dim=-1)
            occ_targets = occ_targets.squeeze(dim=-1)
            one_hot_occ_targets = torch.zeros(
                *list(occ_targets.shape), 2,
                dtype=occ_preds.dtype, device=occ_targets.device
            )
            one_hot_occ_targets.scatter_(-1, occ_targets.unsqueeze(dim=-1).long(), 1.0)
            occ_preds = occ_preds.view(occ.shape[0], -1, 1)
            one_hot_occ_targets = one_hot_occ_targets[..., 1:]
            
            occ_loss_src = self.cls_loss_func(occ_preds,
                                          one_hot_occ_targets,
                                          weights=cls_weights)  # [N, M]
            occ_loss = occ_loss_src.sum() / occ.shape[0]
            occ_conf_loss = occ_loss * self.occ_weight
            conf_loss = conf_loss + occ_conf_loss
