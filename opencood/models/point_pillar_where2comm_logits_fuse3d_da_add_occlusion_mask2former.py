...
class PointPillarWhere2commLogitsFuse3DDAAddOcclusionMask2Former(nn.Module):
    def __init__(self, args):
        super(PointPillarWhere2commLogitsFuse3DDAAddOcclusionMask2Former, self).__init__()
        ...
        self.logits_head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(self.pillar_vfe.get_output_feature_dim()//2, 20),
        )
        self.pillar_feat_down = nn.Sequential(
            nn.Linear(self.pillar_vfe.get_output_feature_dim()+20, self.pillar_vfe.get_output_feature_dim()),
        ...
        self.logits_atten = nn.Sequential(
                nn.Linear(self.pillar_vfe.get_output_feature_dim()+20+20, self.pillar_vfe.get_output_feature_dim()//2),
                nn.BatchNorm1d(self.pillar_vfe.get_output_feature_dim()//2),
                nn.GELU(),
        ...
        self.logits_img_da = nn.Sequential(
            nn.Conv2d(20, self.pillar_vfe.get_output_feature_dim()//2, kernel_size=1),
            DeformableAttention(
                ...
                dropout = 0.,                # dropout
                downsample_factor = 8,       # downsample factor (r in paper)
                offset_scale = None,            # scale of offset, maximum offset
                offset_groups = None,        # number of offset groups, should be multiple of heads
                offset_kernel_size = 8,      # offset kernel size
            ),
    ...
    def forward(self, data_dict, vis=False, fuse_2d=True):
        ...
        layer_out = torch.cat(layer_out, dim=0) # 12*100*20
        layer_out = layer_out.reshape(-1, 20, 1, 1)
        layer_out = self.logits_img_da(layer_out) + layer_out #64*19*1*1
        sem_logits_feature = []
        uv_nonzero_all = []
