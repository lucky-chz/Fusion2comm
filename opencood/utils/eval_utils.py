...
def caluclate_tp_fp(det_boxes, det_score, gt_boxes, result_stat, iou_thresh):
    ...
    if det_boxes is not None:
        ...
        det_polygon_list = list(common_utils.convert_format(det_boxes))                                 # iou on bev
...
def caluclate_tp_fp_score(det_boxes, det_score, gt_boxes, result_stat, iou_thresh):
    """
    Calculate the true positive and false positive numbers of the current
    frames.
    ...
    if det_boxes is not None:
        ...
        for i in range(score_order_descend.shape[0]):
            ...
            if len(gt_polygon_list) == 0 or np.max(ious) < iou_thresh:
                fp.append(1)
                tp.append(0)
                score.append(det_score[score_order_descend[i]])
                continue
    ...
    result_stat[iou_thresh]['fp'] += fp
    result_stat[iou_thresh]['tp'] += tp
    result_stat[iou_thresh]['score'] += score
    result_stat[iou_thresh]['gt'] += gt
...
def calculate_ap_score(result_stat, iou):
    """
    Calculate the average precision and recall, and save them into a txt.
    ...
    score_order_descend = np.argsort(-score)
    fp = list(np.array(fp)[score_order_descend])
    tp = list(np.array(tp)[score_order_descend])
    score = list(score[score_order_descend])
    assert len(fp) == len(tp)
...
def eval_final_results_score(result_stat, save_path, noise_level=None):
    dump_dict = {}

    ap_30, mrec_30, mpre_30 = calculate_ap_score(result_stat, 0.30)
    ap_50, mrec_50, mpre_50 = calculate_ap_score(result_stat, 0.50)
    ap_70, mrec_70, mpre_70 = calculate_ap_score(result_stat, 0.70)
