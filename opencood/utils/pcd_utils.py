...
def mask_ego_points(points, return_mask=False):
    """
...
def shuffle_points(points, return_shuffle_idx=False):
    shuffle_idx = np.random.permutation(points.shape[0])
    points = points[shuffle_idx]
    if return_shuffle_idx:
        return points, shuffle_idx
    return points

def inImage(uv, image_H, image_W):
    u = uv[:, 0]
    v = uv[:, 1]
    mask_u = (u>=0) & (u<image_W)
    mask_v = (v>=0) & (v<image_H)
    mask = mask_u & mask_v
    return mask

def camera2uv(camera_pc, intrinsic):
    """
        camera_pc: N, 4
        intrinsic
    """
    xyz = camera_pc[:, :3]
    intensity = camera_pc[:, 3]
    uvz = np.matmul(intrinsic, xyz.transpose(1, 0)).transpose((1, 0))
    uv = uvz[:, [0, 1]]/uvz[:, [2]]
    return uv
