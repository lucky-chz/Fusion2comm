...
def main():
    ...
    result_stat = {0.3: {'tp': [], 'fp': [], 'score': [], 'gt': 0},
                   0.5: {'tp': [], 'fp': [], 'score': [], 'gt': 0},
                   0.7: {'tp': [], 'fp': [], 'score': [], 'gt': 0}}
    ...
    for i, batch_data in enumerate(tqdm(data_loader)):
        with torch.no_grad():
            ...
            eval_utils.caluclate_tp_fp_score(pred_box_tensor,
                                       pred_score,
                                       gt_box_tensor,
                                       result_stat,
                                       0.3)
    ...
    if len(total_comm_rates) > 0:
        ...
        comm_rates = 0
    ap_30, ap_50, ap_70 = eval_utils.eval_final_results_score(result_stat, opt.model_dir)
