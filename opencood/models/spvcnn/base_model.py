...
class LightningBaseModel(pl.LightningModule):
    def __init__(self, args):
        ...
        # self.txt_logger = self.get_txt_logger()

    # def get_txt_logger(self):
    #     self.txt_logger = logging.getLogger("logger")
    #     self.txt_logger.setLevel(logging.INFO)
    #     txt_log_dir = self.logger.log_dir
    #     log_file = os.path.join(txt_log_dir,"model.log")
    ...
    #     # 将处理器，添加至日志器中
    #     self.txt_logger.addHandler(sh)
    #     self.txt_logger.addHandler(fh)
    #     return self.txt_logger
        ...
        if self.args['train_params']["lr_scheduler"] == 'StepLR':
            ...
            lr_scheduler = torch.optim.lr_scheduler.LambdaLR(
                optimizer, lr_lambda=partial(
                    ...
                    num_gpu=len(self.args.gpu) * self.args.num_node
                ),
            )
        else:
            raise NotImplementedError
    ...
    def validation_step(self, data_dict, batch_idx):
        ...
        now_device = data_dict['logits'].device
        val_total_num = len(self.val_dataloader().dataset)
        hist_tensor = torch.zeros(val_total_num, self.val_iou.unique_label.shape[0], self.val_iou.unique_label.shape[0]).to(now_device).long()
        valid_tensor = torch.zeros((val_total_num)).to(now_device).int()
        batch_size = data_dict["batch_idx"].max() + 1
        pre_hist_list = deepcopy(self.val_iou.hist_list)
        self.val_iou.hist_list = []
        for bidx in range(0, batch_size):
            b_mask = (data_dict["batch_idx"] == bidx)
            prediction = prediction.to(b_mask.device)
            raw_labels = raw_labels.to(b_mask.device)
            self.val_iou(
                prediction[b_mask].cpu().detach().numpy(),
                raw_labels[b_mask].cpu().detach().numpy(),
            )
        now_hist_list = torch.from_numpy(np.array(self.val_iou.hist_list)).to(now_device).long()
        
        hist_tensor[data_dict["index"].long(), :, :] = now_hist_list
        valid_tensor[data_dict["index"].long()] = 1
        dist.all_reduce(hist_tensor, op=torch.distributed.ReduceOp.SUM)
        dist.all_reduce(valid_tensor, op=torch.distributed.ReduceOp.SUM)
        hist_list_all = [] + pre_hist_list
        for i in range(0, hist_tensor.shape[0]):
            if valid_tensor[i]<=0:
                continue
            hist_list_all.append(hist_tensor[i].detach().cpu().numpy())
        self.val_iou.hist_list = deepcopy(hist_list_all)
        return data_dict['loss']
    ...
    def test_step(self, data_dict, batch_idx):
        ...
        if not self.args['submit_to_server']:
            ...
            if self.args['dataset_params']['pc_dataset_type'].find("nuScenes")<=-1:
                ...
                full_save_value_dir = os.path.join(self.submit_dir, 'sequences_value', sequence, 'predictions_value')
                os.makedirs(full_save_value_dir, exist_ok=True)
                np.save(os.path.join(full_save_value_dir, label_name), vote_logits.cpu().numpy()) 
            else:
                meta_dict = {
                    "meta": {
                        "use_camera": False,
                ...
                zero_mask = (prediction == 0)
                prediction[zero_mask] = vote_logits[zero_mask, 1:].argmax(1) + 1
    ...
    def validation_epoch_end(self, outputs):
        ...
        # self.txt_logger.info(str_print)
