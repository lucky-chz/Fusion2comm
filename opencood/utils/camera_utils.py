...
def project_3d_to_2d(gt_box3d, int_matrix, ext_matrix, image_H=600, image_W=800, image=None, idx=None):
    """
    Projects XYZ points onto the canvas and returns the projected canvas
    coordinates.
    ...
    Args:
        gt_box3d : np.ndarray
            shape (N, 3). point coord in world (LiDAR) coordinate. 
        int_matrix : np.ndarray
            shape (4, 4)
        ext_matrix : np.ndarray
            shape (4, 4), T_wc, transform point in camera coord to world coord.
    ...
    Returns:
        gt_box2d : np.ndarray
            shape (N, 2). pixel coord (u, v) in the image. You may want to flip them for image data indexing. 
        gt_box2d_mask : np.ndarray (bool)
            shape (N,). If false, this box is out of image boundary
        fg_mask : np.ndarray 
            shape (image_H, image_W), 1 means foreground, 0 means background
    ...
    gt_box2d = uv_int.reshape(N, 2) # [N, 2]
    gt_box2d_u = np.clip(gt_box2d[:,0], 0, image_W-1)
    gt_box2d_v = np.clip(gt_box2d[:,1], 0, image_H-1)
    gt_box2d = np.stack((gt_box2d_u, gt_box2d_v), axis=-1) # [N, 2]
