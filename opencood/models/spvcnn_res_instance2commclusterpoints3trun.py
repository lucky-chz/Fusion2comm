...
from opencood.models.instance_center_modules.instance_center import InstanceCenter, InstanceComm, InstanceCommClusterPoints
...
class SPVCNNResInstance2commClusterPoints3trun(nn.Module):
    def __init__(self, args, train=True):
        super(SPVCNNResInstance2commClusterPoints3trun, self).__init__()
        ...
        self.instance2comm = InstanceCommClusterPoints(args['InstanceComm'])
        ...
        self.voxel_generator_cluster = PointToVoxel(
            vsize_xyz=args['voxel_size'],
            coors_range_xyz=args['lidar_range'],
            max_num_points_per_voxel=args['max_points_per_voxel'],
            max_num_voxels=args['max_voxel_train'] if train else args['max_voxel_test'],
            num_point_features=args['num_point_features']+4, # 这里的feature数目要改掉 改成6
        )
        # PIllar VFE
        ...
        self.cluster_down = nn.Linear(4+args['InstanceComm']['input_dims']+20, args['pillar_vfe']['num_filters'][-1])
    ...
    def sp_voxel_gen(self, data_dict, side='all', filter_pc=False):
        ...
        return data_dict
    def sp_cluster_voxel_gen(self, data_dict, side='all', filter_pc=False):
        batch_size = data_dict['batch_size']
        voxel_all = []
        voxel_coords_all = []
        voxel_num_points_all = []
        for b_idx in range(0, batch_size):
            ...
            if filter_pc:
                ...
                mask_b = mask_proj & mask_b
            pc = data_dict['points'][mask_b]
            pc_cluster_center = pc.clone()
            pc_cluster_center[:, 0:pc.shape[1]] = data_dict['points_cluster_center'][mask_b][:, 0:pc.shape[1]]
            middle_feature = data_dict['points_feature'][mask_b]
            logits = data_dict['logits'][mask_b]
            pc_cluster_logits = data_dict['points_cluster_logits'][mask_b].to(logits)
            # print(pc.shape)
            pcd_feat = torch.cat([pc, pc_cluster_center, middle_feature, pc_cluster_logits], dim=1)
            if pcd_feat.shape[0]<=0:
                continue
            voxel_output = self.voxel_generator_cluster(pcd_feat)
            if isinstance(voxel_output, dict):
                voxels, coordinates, num_points = \
                voxel_output['voxels'], voxel_output['coordinates'], \
                voxel_output['num_points_per_voxel']
        ...
        data_dict['voxel_num_points'] = voxel_num_points_all
        return data_dict
    def instance2comm_cluster_vfe(self, batch_dict):
        voxel_features, voxel_num_points, coords = \
            batch_dict['voxel_features'], batch_dict['voxel_num_points'], \
            batch_dict['voxel_coords']
        features_mean = \
            voxel_features[:, :, :].sum(dim=1, keepdim=True) / \
            voxel_num_points.type_as(voxel_features).view(-1, 1, 1)
        batch_dict['pillar_features'] = self.cluster_down(features_mean.squeeze(1))
        return batch_dict
    def regroup(self, x, record_len):
        cum_sum_len = torch.cumsum(record_len, dim=0)
        split_x = torch.tensor_split(x, cum_sum_len[:-1].cpu())
    ...
    def forward(self, data_dict, vis=False):
        ...
        # psm_single_max_pool = F.max_pool2d(psm_single, kernel_size=2, stride=1, padding=1)
        # psm_single_max_pool = psm_single_max_pool[:, :, 0:psm_single.shape[2], 0:psm_single.shape[3]]
        # psm_single_score = psm_single.sigmoid()
        # psm_single_low_mask = psm_single_score>self.args['fusion_args']['communication']['thre']*1.00
        # psm_single_max_mask = (((psm_single_max_pool - psm_single)<=0.005) & psm_single_low_mask) #| (psm_single_score>self.args['fusion_args']['communication']['thre'])
        psm_single_infuse = psm_single.clone()
        # psm_single_infuse[psm_single_max_mask] = 10000
        # psm_single_infuse[~psm_single_max_mask] = -10000
        ...
        data_dict_vi_cluster = self.sp_cluster_voxel_gen(data_dict_vi_cluster, side='all', filter_pc=False)
        ...
        if data_dict_vi_cluster['empty'] is False:
            ...
            # n, 4 -> n, c
            batch_dict = self.instance2comm_cluster_vfe(batch_dict)
            # n, c -> N, C, H, W
            batch_dict = self.scatter(batch_dict)
        else:
            batch_dict = {}
