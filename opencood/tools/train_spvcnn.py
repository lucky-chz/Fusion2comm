...
import statistics
import sys
sys.path.append('/space/chuhz/workspace/v2x_object/Where2comm')
import torch
...
def load_pretrained_model(model, pretrain_model_path):
        ...
        for k, v in pretrained_dict.items():
            if k in model_dict.keys():
                if model_dict[k].shape == v.shape:
                    pretrain_match_dict[k] = v
        model_dict.update(pretrain_match_dict)
...
def main():
    ...
    if opt.pretrain_model is not None:
        pretrain_model_path = opt.pretrain_model
        load_pretrained_model(model, pretrain_model_path)
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            if 'psm_ego_i' in output_dict:
                final_loss = final_loss + criterion(output_dict, batch_data['ego']['label_dict'], prefix='_ego_i')
            if 'points_seg_loss' in output_dict:
                final_loss = final_loss + output_dict['points_seg_loss']
            if 'cls_sim_loss' in output_dict:
                final_loss = final_loss + output_dict['cls_sim_loss']
            if 'logits_kl_loss' in output_dict:
                final_loss = final_loss + output_dict['logits_kl_loss']
    ...
    if run_test:
        ...
        cmd = f"python /space/chuhz/workspace/v2x_object/Where2comm/opencood/tools/inference.py --model_dir {saved_path} --fusion_method {fusion_method}"
