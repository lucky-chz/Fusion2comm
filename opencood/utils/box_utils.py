...
def corner_get_theta(corner3d):
    vector = np.zeros((corner3d.shape[0], 2))
    vector[:, 0] = (corner3d[:, 1, 1] - corner3d[:, 2, 1]) + \
                   (corner3d[:, 0, 1] - corner3d[:, 3, 1]) + \
                   (corner3d[:, 5, 1] - corner3d[:, 6, 1]) + \
                   (corner3d[:, 4, 1] - corner3d[:, 7, 1])
    vector[:, 1] = (corner3d[:, 1, 0] - corner3d[:, 2, 0]) + \
                   (corner3d[:, 0, 0] - corner3d[:, 3, 0]) + \
                   (corner3d[:, 5, 0] - corner3d[:, 6, 0]) + \
                   (corner3d[:, 4, 0] - corner3d[:, 7, 0])
    vector = vector/4
    theta = np.arctan2(vector[:, 0], vector[:, 1])[:, np.newaxis]
    return theta
    
def corner_to_center(corner3d, order='lwh', theta_method='forward'):
    """
    ...
    if theta_method == 'default':
        ...
        theta = corner_get_theta(corner3d)
...
def load_single_objects_dairv2x(object_list,
    ...
    for object_content in object_list:        
        ...
        # occlusion_state = float(object_content['occluded_state']) if object_content['occluded_state'] is not None else -1 # 0:fully visible 1:partly occluded 2:largely occluded 3:unknown
...
def load_single_objects_dairv2x_add_occlusion(object_list,
                          output_dict,
                          lidar_range,
                          order):
    """
    ...
    for object_content in object_list:        
        ...
        bbx_lidar = mask_boxes_outside_range_numpy(bbx_lidar, lidar_range_z_larger, order)
        if bbx_lidar.shape[0] > 0:
            bbx_lidar = np.append(bbx_lidar[0], occlusion_state).reshape(1,-1)
            if object_content['type'] == "Car" or \
               object_content['type'] == "Van" or \
               object_content['type'] == "Truck" or \
               object_content['type'] == "Bus":
