...
from opencood.utils.camera_utils import load_camera_data, load_intrinsic_DAIR_V2X, project_3d_to_2d
from opencood.hypes_yaml.yaml_utils import load_yaml
from opencood.utils.pcd_utils import \
    mask_points_by_range, mask_ego_points, shuffle_points, \
    downsample_lidar_minimum, lidar_project, camera2uv
...
def load_data_info(info_path):
    co_datainfo = load_json(info_path)
    co_data = OrderedDict()
    for frame_info in co_datainfo:
        if 'vehicle_image_path' in frame_info:
            veh_frame_id = frame_info['vehicle_image_path'].split("/")[-1].replace(".jpg", "")
        else:
            veh_frame_id = frame_info['image_path'].split("/")[-1].replace(".jpg", "")
        co_data[veh_frame_id] = frame_info
    return co_data
class IntermediateFusionDatasetDAIR_Lidar_Occlusion_SPVCNN(Dataset):
    """
    ...
    def __init__(self, params, visualize, train=True):
        ...
        self.infra_data_info = load_data_info(os.path.join(self.root_dir, 'infrastructure-side/data_info.json'))
        self.vehicle_data_info = load_data_info(os.path.join(self.root_dir, 'vehicle-side/data_info.json'))
    def retrieve_base_data(self, idx):
        ...
        lidar_np_vehicle, _ = pcd_utils.read_pcd(os.path.join(self.root_dir,frame_info["vehicle_pointcloud_path"]))
        lidar_seg_vehicle = np.load(os.path.join(self.root_dir, (frame_info["vehicle_pointcloud_path"].replace('velodyne', 'lidar_sem_seg').replace('pcd', 'npy')))) + 2
        data[0]['lidar_np'] = np.concatenate((lidar_np_vehicle, lidar_seg_vehicle.reshape(-1, 1)), axis=-1)
        # 做投影
        if self.clip_pc:
            data[0]['lidar_np'] = data[0]['lidar_np'][data[0]['lidar_np'][:,0]>0]
        with open(os.path.join(self.root_dir, (frame_info["vehicle_pointcloud_path"].replace('velodyne', 'instance_center').replace('pcd', 'pickle'))), 'rb') as f:
            instance_center_dict = pickle.load(f)
        data[0]['instance_labels'] = instance_center_dict['instance_labels']
        data[0]['instance_center'] = instance_center_dict['instance_center']
        data[0]['pointcloud_timestamp'] = int(self.vehicle_data_info[veh_frame_id]['pointcloud_timestamp'])
        ...
        lidar_np_infra, _ = pcd_utils.read_pcd(os.path.join(self.root_dir,frame_info["infrastructure_pointcloud_path"]))
        lidar_seg_infra = np.load(os.path.join(self.root_dir, (frame_info["infrastructure_pointcloud_path"].replace('velodyne', 'lidar_sem_seg').replace('pcd', 'npy')))) + 2
        data[1]['lidar_np'] = np.concatenate((lidar_np_infra, lidar_seg_infra.reshape(-1, 1)), axis=-1)
        with open(os.path.join(self.root_dir, (frame_info["infrastructure_pointcloud_path"].replace('velodyne', 'instance_center').replace('pcd', 'pickle'))), 'rb') as f:
            instance_center_dict = pickle.load(f)
        data[1]['instance_labels'] = instance_center_dict['instance_labels']
        data[1]['instance_center'] = instance_center_dict['instance_center']
        data[1]['pointcloud_timestamp'] = int(self.infra_data_info[inf_frame_id]['pointcloud_timestamp'])
        if self.load_camera_file:
            data[0]['camera_data'] = load_camera_data([os.path.join(self.root_dir, frame_info["vehicle_image_path"])])
            data[0]['semseg'] = np.load(os.path.join(self.root_dir, (frame_info["vehicle_image_path"].replace('image', 'semseg_mask2former')+'.npy')))
            ...
            data[1]['semseg'] = np.load(os.path.join(self.root_dir, (frame_info["infrastructure_image_path"].replace('image', 'semseg_mask2former')+'.npy')))
    ...
    def generate_object_center_single(self,
                               cav_contents,
                               reference_lidar_pose, return_visible_mask=False):
        return self.post_processor.generate_object_center_dairv2x_late_fusion_add_occlusion(cav_contents)
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose, ego_pose_clean, ego_keypoints, ego_allpoints, idx):
        ...
        # filter lidar
        instance_labels = selected_cav_base['instance_labels']
        instance_center = selected_cav_base['instance_center']
        lidar_np = selected_cav_base['lidar_np']
        # lidar_np = np.concatenate([selected_cav_base['lidar_np'], selected_cav_base['lidar_np_uv']], axis=0) # 4->6
        # 加一个lidar_np_uv  
        lidar_np, shuffle_idx = shuffle_points(lidar_np, return_shuffle_idx=True) 
        instance_labels = instance_labels[shuffle_idx]
        instance_center = instance_center[shuffle_idx]
        # remove points that hit itself
        lidar_np, lidar_mask = mask_ego_points(lidar_np, return_mask=True)
        instance_labels = instance_labels[lidar_mask]
        instance_center = instance_center[lidar_mask]
        # 外参矩阵 selected_cav_base['params']['camera0']['extrinsic']   内参矩阵 selected_cav_base['params']['camera0']['intrinsic']  
        # project the lidar to ego space
        # x,y,z in ego space
        unprojected_lidar = lidar_np[:, :].copy()
        projected_lidar = \
        ...
        pointcloud_timestamp = selected_cav_base['pointcloud_timestamp']
        # camera
        if self.load_camera_file:
            camera_data_list = selected_cav_base["camera_data"]
            segs = torch.from_numpy(selected_cav_base["semseg"])
        ...
        lidar_np = mask_points_by_range(lidar_np,
                                        self.params['preprocess'][
                                            'cav_lidar_range'])
        # 增加 lidar_np_uv = project(lidar_np, selected_cav_base['params']['camera0']['extrinsic'], 内参矩阵 selected_cav_base['params']['camera0']['intrinsic']  )
        # lidar_np = np.concat([lidar_np, lidar_np_uv], axis=1)
        lidar_np_camera = lidar_project(lidar_np, selected_cav_base['params']['camera0']['extrinsic'])
        front_mask = lidar_np[:, 0]>0
        uv_front = camera2uv(lidar_np_camera[front_mask, :], selected_cav_base['params']['camera0']['intrinsic'])
        lidar_np_uv = -10000 * np.ones((lidar_np.shape[0], 2))
        lidar_np_uv[front_mask] = uv_front
        
        # lidar_np_uv = project_3d_to_2d(lidar_np[:,:3], selected_cav_base['params']['camera0']['intrinsic'], selected_cav_base['params']['camera0']['extrinsic'], img.height, img.width)
        lidar_np = np.concatenate([lidar_np, lidar_np_uv], axis=1)
        ...
        selected_cav_processed.update(
            {'object_bbx_center': object_bbx_center[object_bbx_mask == 1],
             ...
             'unprojected_lidar': unprojected_lidar,
    ...
    def get_unique_label_occlusion(self, occlusion_state, object_id_stack_occ):
        object_occlusion_state = np.zeros((self.params['postprocess']['max_num'], 1))
        mask = np.zeros(self.params['postprocess']['max_num'])
        if len(occlusion_state) > 0:
            # exclude all repetitive objects    
            unique_indices = [object_id_stack_occ.index(x) for x in set(object_id_stack_occ)]
            occlusion_state = np.vstack(occlusion_state) if len(occlusion_state) > 1 else occlusion_state[0]
            occlusion_state = occlusion_state[unique_indices]
            object_occlusion_state[:occlusion_state.shape[0], :] = occlusion_state
            mask[:occlusion_state.shape[0]] = 1
        return object_occlusion_state, mask
    ...
    def __getitem__(self, idx):
        ...
        unprojected_lidar_stack = []
        instance_labels_stack = []
        instance_center_stack = []
        ...
        for cav_id in cav_id_list:
            ...
            # lidar_np_lists.append(selected_cav_processed['lidar_np'].tolist())
            object_stack.append(selected_cav_processed['object_bbx_center'])
            object_id_stack += selected_cav_processed['object_ids']            
            ######################## Single View GT ########################
            if cav_id == 0:
                object_stack_single_v.append(selected_cav_processed['object_bbx_center_single'][:,:7])
                occlusion_state_v.append(selected_cav_processed['object_bbx_center_single'][:,7].reshape(-1, 1)) # 车端的遮挡程度
                object_id_stack_single_v += selected_cav_processed['object_ids_single']
            else:
                object_stack_single_i.append(selected_cav_processed['object_bbx_center_single'][:,:7])
                occlusion_state_i.append(selected_cav_processed['object_bbx_center_single'][:,7].reshape(-1, 1)) # 路端的遮挡程度
            ...
            unprojected_lidar_stack.append(
                selected_cav_processed['unprojected_lidar']
            )
            instance_labels_stack.append(
                selected_cav_processed['instance_labels']
            )
            instance_center_stack.append(
                selected_cav_processed['instance_center']
            )
            pointcloud_timestamp.append(
                selected_cav_processed['pointcloud_timestamp']
            )
        ...
        # lidar_np_list = np.array(lidar_np_lists)
        ######################################################
        ...
        occlusion_state_bbox_v, __ = self.get_unique_label_occlusion(occlusion_state_v, object_id_stack_single_v)
        ...
        occlusion_state_bbox_i, __ = self.get_unique_label_occlusion(occlusion_state_i, object_id_stack_single_i)
        ######################## Single View GT ########################
        ...
        if self.load_camera_file:
            merged_image_inputs_dict = self.merge_features_to_dict(agents_image_inputs)
            processed_data_dict['ego'].update({'image_inputs': merged_image_inputs_dict})
            # processed_data_dict['ego'].update({'lidar_np': selected_cav_processed['lidar_np']})
        ...
        label_dict_single_i = \
            self.post_processor.generate_label_add_occ(
                gt_box_center=object_bbx_center_single_i,
                anchors=anchor_box,
                mask=mask_single_i,
                occlusion_state=occlusion_state_bbox_i
                )
        ...
        processed_data_dict['ego'].update(
            {'object_bbx_center': object_bbx_center,
             ...
             'occlusion_state_v': occlusion_state_bbox_v,
             ...
             'occlusion_state_i': occlusion_state_bbox_i,
            ...
            #  'lidar_np': lidar_np_list,
             'lidar_np': np.vstack(selected_cav_processed['lidar_np']),
             })
        ...
        processed_data_dict['ego'].update({
            'origin_unprojected_lidar_v': unprojected_lidar_stack[0],
            'origin_unprojected_lidar_i': unprojected_lidar_stack[1],
            'instance_labels_v': instance_labels_stack[0],
            'instance_labels_i': instance_labels_stack[1],
            'instance_center_v': instance_center_stack[0],
            'instance_center_i': instance_center_stack[1],
            'pointcloud_timestamp': np.array(pointcloud_timestamp).astype(np.int64)
        })
    ...
    def collate_batch_train(self, batch):
        ...
        if self.visualize:
            ...
            origin_lidar_v = []
            origin_lidar_i = []
        origin_unprojected_lidar_v = []
        origin_unprojected_lidar_i = []
        origin_lidar_batchidx_v = []
        origin_lidar_batchidx_i = []
        instance_labels_v = []
        instance_labels_i = []
        instance_center_v = []
        instance_center_i = []
        ...
        for i in range(len(batch)):
            ...
            occlusion_state_v.append(ego_dict['occlusion_state_v'])
            ...
            occlusion_state_i.append(ego_dict['occlusion_state_i'])
            ######################## Single View GT ########################
            ...
            # lidar_np_list.append(ego_dict['lidar_np']) 
            lidar_np_list.append(torch.from_numpy(ego_dict['lidar_np']) )
            ...
            origin_unprojected_lidar_v.append(ego_dict['origin_unprojected_lidar_v'])
            origin_unprojected_lidar_i.append(ego_dict['origin_unprojected_lidar_i'])
            origin_lidar_batchidx_v.append(np.ones(ego_dict['origin_unprojected_lidar_v'].shape[0])*i)
            origin_lidar_batchidx_i.append(np.ones(ego_dict['origin_unprojected_lidar_i'].shape[0])*i)
            instance_labels_v.append(ego_dict['instance_labels_v'])
            instance_labels_i.append(ego_dict['instance_labels_i'])
            instance_center_v.append(ego_dict['instance_center_v'])
            instance_center_i.append(ego_dict['instance_center_i'])
            pointcloud_timestamp.append(ego_dict['pointcloud_timestamp'])
        ...
        occlusion_state_v = torch.from_numpy(np.array(occlusion_state_v))
        ...
        occlusion_state_i = torch.from_numpy(np.array(occlusion_state_i))
        ######################## Single View GT ########################
        ...
        if self.load_camera_file:
            merged_image_inputs_dict = self.merge_features_to_dict(image_inputs_list)
        ...
        # lidar_np_list = torch.from_numpy(np.concatenate(lidar_np_list, axis=0))
        ...
        output_dict['ego'].update({'object_bbx_center': object_bbx_center,
                                   ...
                                   'occlusion_state_v': occlusion_state_v,
                                   ...
                                   'occlusion_state_i': occlusion_state_i,
        ...
        if self.visualize:
            ...
            output_dict['ego'].update({'origin_projected_lidar_v': torch.from_numpy(np.concatenate(origin_lidar_v, axis=0)),
                                       'origin_projected_lidar_i': torch.from_numpy(np.concatenate(origin_lidar_i, axis=0))})
            origin_lidar_v = \
        ...
        output_dict['ego'].update({'origin_unprojected_lidar_v': torch.from_numpy(np.concatenate(origin_unprojected_lidar_v, axis=0)),
                                   'origin_unprojected_lidar_i': torch.from_numpy(np.concatenate(origin_unprojected_lidar_i, axis=0)),
                                   'origin_lidar_batchidx_v': torch.from_numpy(np.concatenate(origin_lidar_batchidx_v, axis=0).astype(np.int64)),
                                   'origin_lidar_batchidx_i': torch.from_numpy(np.concatenate(origin_lidar_batchidx_i, axis=0).astype(np.int64)),
                                   'instance_labels_v': torch.from_numpy(np.concatenate(instance_labels_v, axis=0).astype(np.int64)),
                                   'instance_labels_i': torch.from_numpy(np.concatenate(instance_labels_i, axis=0).astype(np.int64)),
                                   'instance_center_v': torch.from_numpy(np.concatenate(instance_center_v, axis=0).astype(np.float32)),
                                   'instance_center_i': torch.from_numpy(np.concatenate(instance_center_i, axis=0).astype(np.float32)),
                                   'pointcloud_timestamp': torch.from_numpy(np.concatenate(pointcloud_timestamp, axis=0).astype(np.int64)),
                                   })
