...
from opencood.models.comm_modules.where2comm_occ import Communication_occ


class ScaledDotProductAttention_occ(nn.Module):
    """
    ...
    def __init__(self, dim):
        super(ScaledDotProductAttention_occ, self).__init__()
...
class AttenFusion(nn.Module):
    def __init__(self, feature_dim):
        ...
        self.att = ScaledDotProductAttention_occ(feature_dim)
...
class Where2comm_occ(nn.Module):
    def __init__(self, args):
        super(Where2comm_occ, self).__init__()

        self.communication = False
        self.round = 1
        if 'communication' in args:
            self.communication = True
            self.naive_communication = Communication_occ(args['communication'])
        ...
        self.multi_scale_mask = False
        self.multi_scale_byte = False
        if 'multi_scale_mask' in args:
            self.multi_scale_mask = args['multi_scale_mask']
        if 'multi_scale_byte' in args:
            self.multi_scale_byte = args['multi_scale_byte']
        if self.multi_scale:
    ...
    def forward(self, x, rm, occ, record_len, pairwise_t_matrix, backbone=None, heads=None):
        """
        Fusion forwarding.
            ...
            for i in range(self.num_levels):
                ...
                if i==0:
                    if self.communication:
                        ...
                        batch_occlusion_maps = self.regroup(occ, record_len)
                        _, communication_masks, communication_rates = self.naive_communication(batch_confidence_maps, batch_occlusion_maps, record_len, pairwise_t_matrix)
                        x = x * communication_masks
                    else:
                        communication_rates = torch.tensor(0).to(x.device)
                    if self.multi_scale_byte:
                        _, c_nl, h_nl, w_nl = x.shape
                        communication_rates = communication_rates * h_nl * w_nl * c_nl
                elif self.multi_scale_mask:
                    # add for other level
                    if self.communication:
                        _, _, h_nl, w_nl = x.shape
                        rm_nl = transforms.Resize((h_nl, w_nl))(rm)
                        occ_nl = transforms.Resize((h_nl, w_nl))(occ)
                        batch_confidence_maps = self.regroup(rm_nl, record_len)
                        batch_occlusion_maps = self.regroup(occ_nl, record_len)
                        _, communication_masks, communication_rates_nl = self.naive_communication(batch_confidence_maps, batch_occlusion_maps, record_len, pairwise_t_matrix)
                        x = x * communication_masks
                    else:
                        communication_rates_nl = torch.tensor(0).to(x.device)
                    if self.multi_scale_byte:
                        _, c_nl, h_nl, w_nl = x.shape
                        communication_rates_nl = communication_rates_nl * h_nl * w_nl * c_nl
                    communication_rates = communication_rates + communication_rates_nl
